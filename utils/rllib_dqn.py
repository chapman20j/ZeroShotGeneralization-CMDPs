...
# NOTE: I denote edits to this file with `# NOTE: edited` in the comments.
# NOTE: This file is from adapted RLlib:
...
ALLOWED_FAILS_PER_RB_SAMPLE = 5  # NOTE: edited
ALLOWED_FAILS_TOTAL = 100  # NOTE: edited
...
class DQNConfig(AlgorithmConfig):
    ...
    def training(
        ...
        *,
        ...
        include_infos: bool = False,  # NOTE: edited
        **kwargs,
    ) -> "DQNConfig":
...
class DQN(Algorithm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.total_rb_fails = 0  # NOTE: edited
        ...
        if current_ts >= self.config.num_steps_sampled_before_learning_starts:
            ...
            for _ in range(sample_and_train_weight):
                ...
                with self.metrics.log_time((TIMERS, REPLAY_BUFFER_SAMPLE_TIMER)):
                    # NOTE: I'm modifying the following to catch errors
                    episodes = None
                    # NOTE: edited
                    for this_iter in range(ALLOWED_FAILS_PER_RB_SAMPLE):
                        try:
                            episodes = self.local_replay_buffer.sample(
                                ...
                                * self.config.model_config.get("max_seq_len", 0),
                                ...
                                include_infos=self.config.include_infos,
                            )
                            # break if successful
                            break
                        except KeyError as e:
                            self.total_rb_fails += 1
                            if self.total_rb_fails >= ALLOWED_FAILS_TOTAL:
                                raise KeyError(
                                    f"Replay buffer sample failed {self.total_rb_fails} times. "
                                    f"Last error: {e}"
                                )
                            if this_iter == ALLOWED_FAILS_PER_RB_SAMPLE - 1:
                                raise KeyError(
                                    f"Replay buffer sample failed {this_iter} times in a row. "
                                    f"Last error: {e}"
                                )
